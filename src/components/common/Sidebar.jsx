...
const Sidebar = ({ open, close }) => {
  ...
  const rawRole = (user.rol || user.ROL || "").toUpperCase();
  
  const menu = menuConfig[rawRole] || menuConfig["VECINO"] || [];
        ...
        <div className="px-4 py-6">
          <div className="mb-8 px-4 py-3 bg-gray-50 rounded-2xl border border-gray-100 flex items-center gap-3">
             ...
             <div>
                ...
                <p className="text-xs font-bold text-gray-700">{rawRole}</p>
             </div>
          </div>
          ...
          <nav className="space-y-1">
            ...
            {menu.length === 0 ? (
              <div className="px-4 py-3 bg-amber-50 rounded-xl border border-amber-100">
                <p className="text-[10px] text-amber-600 font-bold uppercase">Configuración pendiente</p>
                <p className="text-[10px] text-amber-500 italic">No hay opciones para: {rawRole}</p>
              </div>
            ) : (
              menu.map((item) => (
                <NavLink
